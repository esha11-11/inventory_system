     ...
     <div class="box">
            <div class="box-header with-border">
              <h3 class="box-title"> 
                    ...
                    <a class="btn btn-default btn-xm " href="{{ route('media.create') }}"><i class="fa fa-plus"></i></a>
              </h3>
              <div class="box-tools">
            ...
            <div class="box-body">
              <table class="table table-bordered">
                ...
                <tr>
                  <td><input type="checkbox" name="checkAll[]" id="" value="{{ $media->id }}" class="checkSingle"></td>
                  <td>{{$media->title}}</td>
                  <td>{{$media->media_type}}</td>

                  <td>
                    @if($media->media_img == 'No image found')
                    <img src="/admin_imgs/no_img.png"  width="100" height="100" alt="{{ $media->title }}">
                    @else
                    <img src="/admin_imgs/media/{{ $media->media_img }}"  width="100" height="100" alt="{{ $media->title }}">
                    @endif
                  </td>
                  <td>
    ...
    $(document).ready(function() {
      ...
      $("#activeAllStatus").on('click',  function(event) {
        ...
        if ($(".checkSingle:checked").length > 0 ) 
        {
          ...
          $.get('{{ route('media.status.active') }}', formSerials, function(data) {
            if (data > 0) 
            {
      ...
      $("#deactiveAllStatus").on('click',  function(event) {
        ...
        {
          var formSerials = $("#formView").serialize();
          $.get('{{ route('media.status.deactive') }}', formSerials, function(data) {
            if (data > 0) 
              window.location.href = '/admin/media';
          });
        }
      ...
      $("#deleteAll").on('click',  function(event) {
        ...
        if ($(".checkSingle:checked").length > 0 ) 
        {
          ...
          {
            var formSerials = $("#formView").serialize();
            $.get('{{ route('media.delete.all') }}', formSerials, function(data) {
            if (data > 0) 
              window.location.href = '/admin/media';
            });
          }
